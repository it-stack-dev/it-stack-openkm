# Troubleshooting — IT-Stack OPENKM
...
docker compose logs --tail=50 openkm

# Follow logs
docker compose logs -f openkm

# Exec into container
docker compose exec openkm bash
```
...
1. Check logs: docker compose logs openkm
...
```bash
# Test connectivity
psql -h lab-db1 -U openkm_user -d openkm_db -c '\conninfo'
...
1. Check resource usage: docker stats it-stack-openkm
...
| Log | Path |
|-----|------|
| Application | docker compose logs openkm |
